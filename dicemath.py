...
DICE_HEIGHT = 5
CANVAS_WIDTH = 79
CANVAS_HEIGHT = 24 - 3 # -3 for rooom to enter the sum at the bottom.
...
while time.time() < startTime + QUIZ_DURATION:
    ...
    # Contains (x, y) tuples of the top-left corener of each die.
    topLeftDiceCorners = []
    ...
    for i, (dieLeft, dieTop) in enumerate(topLeftDiceCorners):
        ...
        for dx in range(DICE_WIDTH):
            for dy in range(DICE_HEIGHT):
                ...
                canvasX = dieLeft + dx
                canvasY = dieTop + dy
                # Note that in dieface, a list of strings, the x and y are swappped:
